...
from .VP_ReplayBuffer import VP_ReplayBuffer
...
class VP_DDPG(OffPolicyRLModel):
    def __init__(self, policy, env, gamma=0.99, memory_policy=None, eval_env=None, nb_train_steps=50,
        ...
        super(VP_DDPG, self).__init__(policy=policy, env=env, replay_buffer=None,
    ...
    def setup_model(self):
        with SetVerbosity(self.verbose):
            ...
            with self.graph.as_default():
                ...
                self.replay_buffer = VP_ReplayBuffer(self.buffer_size)
    ...
    def _store_transition(self, obs, action, reward, next_obs, done, info):
        ...
        self.replay_buffer.add(obs, action, reward, next_obs, float(done), self.env.env_method('_get_turbulence')[0])
        if self.normalize_observations:
    ...
    def _train_step(self, step, writer, log=False):
        ...
        """
        # Get a batch
        obs, actions, rewards, next_obs, terminals = self.replay_buffer.sample(vol=self.env.env_method('_get_turbulence')[0], batch_size=self.batch_size, env=self._vec_normalize_env)
    ...
    def _adapt_param_noise(self):
        ...
        obs, *_ = self.replay_buffer.sample(vol=self.env.env_method('_get_turbulence')[0], batch_size=self.batch_size, env=self._vec_normalize_env)
